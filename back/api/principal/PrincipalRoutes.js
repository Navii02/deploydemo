...
router.post('/principalregister', async (req, res) => {
  ...
  const user = await PrincipalSchema.findOne({ email }) // finding user in db
  if (user) return res.status(400).json({ msg: 'User already exists' })

  const newUser = new PrincipalSchema({ name,email, password })
  // hasing the password
...
router.post(`/principallogin`, async (req, res) => {
  ...
  const user = await PrincipalSchema.findOne({ email: email }) // finding user in db
  if (!user) {
