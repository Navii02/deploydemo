...
const ApprovedAndRemoved = () => {
  ...
  useEffect(() => {
    axios.get(`${baseurl}/api/officerstudent/approvedStudents`)
      .then(response => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${baseurl}/api/officerstudent/updateStudent/${studentId}`, formData);
      setIsSuccess(true);
      setEditMode(false);
  ...
  const handlePrintPreview = async (_id, photoPath) => {
    try {
      const photoUrl = `${baseurl}/api/officerstudent/image/${encodeURIComponent(photoPath)}`;
      const response = await axios.get(`${baseurl}/api/officerstudent/approvedstudentDetails/${_id}`);
